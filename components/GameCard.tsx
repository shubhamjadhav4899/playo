        ...
        <View className='mt-2.5'>
            <View className='flex-row items-center'>
                <View className='flex-row'>
                    <Image className='w-14 h-14 rounded-full' source={{uri:game.adminUrl}}/>
                    <View className='flex-row items-center -ml-1.5'>
                        {game.players.filter((player)=>player.name!==game.adminName).map((player,idx)=>(
                            <Image className='w-11 h-11 rounded-full -ml-1.5 border border-white' key={idx} source={{uri:player.imageUrl}}/>
                        ))}
                    </View>
                </View>
                <View className='ml-2.5 flex-1'>
                    <Text className='text-base font-medium'>* {game.players.length}/{game.totalPlayers} Going</Text>
                </View>
            ...
            <View className='flex-row justify-between items-center mt-2.5'>
                ...
                {game.matchFull&&(
                    ...
                    className='w-[100px] h-[70px]' resizeMode='contain'/>
                )}
            </View>
